...
resource "null_resource" "db_setup" {
  depends_on = ["aws_db_instance.postgres", "aws_security_group.rds-sg"]

  provisioner "local-exec" {
    command = <<EOT
      PGPASSWORD="${var.db_password}" psql -h ${aws_db_instance.postgres.address} -U postgresdb -d postgresdb -c "
        CREATE TABLE IF NOT EXISTS terraformMetrics (
          id TEXT PRIMARY KEY, 
          success BOOLEAN NOT NULL, 
          timestamp TIMESTAMPTZ DEFAULT NOW(),
          applied_resources TEXT[], 
